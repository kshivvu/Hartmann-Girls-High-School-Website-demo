    ...
    <main>
      ...
      <section className="stats">
        <div className="shell stats-grid">
          <div><strong>95%</strong><span>2025-26 Top Score</span></div>
          <div><strong>67+</strong><span>Years of Excellence</span></div>
          <div><strong>1959</strong><span>Established in Patna</span></div>
      ...
      <section className="quick shell">
        ...
        <div className="quick-links"><Link href="/news">Notices & Announcements →</Link><Link href="/contact">Visit Us — Digha, Patna →</Link><Link href="/academics">Academic Information →</Link></div>
      </section>

      <section className="about-section">
        <div className="shell about-grid">
          <div className="about-photo"><Image src="/images/about-campus.jpg" alt="Hartmann Girls' High School community" fill sizes="(max-width: 800px) 100vw, 50vw" /></div>
      ...
      <section className="why shell section">
        <div className="section-heading"><p className="eyebrow">The Hartmann difference</p><h2>An education grounded<br />in <em>character.</em></h2></div>
      ...
      <section className="toppers section">
        <div className="shell">
          <div className="split-heading"><div><p className="eyebrow">Our toppers</p><h2>Quiet effort.<br /><em>Remarkable results.</em></h2></div><Link className="text-link light" href="/results">View all achievements <span>→</span></Link></div>
          <div className="topper-grid">{toppers.map((topper, index) => <article className={index === 0 ? "topper-card featured" : "topper-card"} key={`${topper.name}-${topper.year}`}>
      ...
      <section className="principal">
        <div className="shell principal-grid">
          <div className="principal-photo"><Image src="/images/principal.jpg" alt="Sr. Mary Vidhya, SND" fill sizes="(max-width: 700px) 100vw, 38vw" /></div>
          <div><p className="eyebrow">From the principal&apos;s desk</p><h2>Formation of character,<br /><em>conscience and compassion.</em></h2><p className="principal-intro">Under the leadership of Sr. Mary Vidhya, SND, Hartmann continues its commitment to an education grounded in faith, discipline and service.</p><p className="signature">Sr. Mary Vidhya, SND <small>Principal, Hartmann Girls&apos; High School</small></p></div>
        </div>
      </section>

      <section className="facilities shell section">
        <div className="section-heading"><p className="eyebrow">School infrastructure</p><h2>Spaces that invite<br /><em>curiosity.</em></h2></div>
        <div className="facility-grid">{facilities.map(([title, copy, number, image]) => <article key={title}><div className="facility-photo"><Image src={image} alt="" fill sizes="(max-width: 800px) 50vw, 33vw" /></div><div className="facility-copy"><span>{number}</span><h3>{title}</h3><p>{copy}</p></div></article>)}</div>
      ...
      <section className="notice-section">
        <div className="shell notice-grid"><div><p className="eyebrow">Notice board</p><h2>Latest from<br /><em>Hartmann.</em></h2><Link className="text-link" href="/news">All notices <span>→</span></Link></div><div>{notices.map((notice, index) => <Link href="/admissions" key={notice} className="notice"><small>ADMISSIONS · 2026-27</small><h3>{notice}</h3><span>Read notice →</span><b>0{index + 1}</b></Link>)}</div></div>
      </section>

      <section className="gallery-section section shell">
        <div className="split-heading"><div><p className="eyebrow">Life at Hartmann</p><h2>Moments from<br /><em>our community.</em></h2></div><Link className="text-link" href="/gallery">View all photographs <span>→</span></Link></div>
